...
class ScreenCtn {
  ...
  animation_msgs(scrollPosn) {
    ...
    for (let i = 0; i < this.msgs.length; i++) {  
      if (scrollPosn >= this.frameLen * (i + this.buffer) 
  ...
  animation_file(scrollPosn) {
    ...
    } else if (scrollPosn >= this._framePosnFromLast(2) && scrollPosn < this._framePosnFromLast(1)) {
  ...
  _screenUnlock() { this.unlockScreen.classList.remove('locked') }
  _screenLock() { this.unlockScreen.classList.add('locked') }
  _framePosnFromLast(n) { return this.frameLen * (this.msgs.length + this.buffer - n) }
...
class Mobile {
  ...
  animation(scrollPosn) {
    ...
    } else if (scrollPosn > this.frameLen * 8 && scrollPosn <= this.frameLen * 10) {
      this._addClass("animation")
      this._removeClass("animation-rotate")
    } else if (scrollPosn > this.frameLen * 10 && scrollPosn <= this.frameLen *11) {
  ...
  _removeClass(className) {
    this.figure.classList.contains(className) && this.figure.classList.remove(className)
  }
}
...
export class WindowScrollService {
   ...
   *  父頁面卷軸時，不要捲軸到子頁面
   */
  prevnetOverlapScroll() { /* prevent scroll the inside div before outside one sroll to bottom */
